...
縦書きの台本は横に長くなるので、ページ送りのボタン（‹ ›）があります。マウスを画面の左端・右端の列（その画面の最後・最初の行）に持っていくと、その側に出ます（いつも出ていると本文に重なるため）。‹ で次のページ（左）へ、› で前のページ（右）へ、見えている幅の 4/5 ぶん動きます（前のページの最後の列が見えたまま残るので、続きを追えます）。場面の最後（左端）・先頭（右端）まで来たほうのボタンは消えます。
...
- **PDF**: Word と同じ用紙・書き方・表紙の情報で、Word を経由せずに直接組んだ PDF。文字はすべてアウトライン化し（フォントを埋め込まない）、色は墨 1 色なので、印刷所への版下にそのまま使えます。1 行の文字数は「設定 › 書式」の人物名欄と本文の文字数に従います（用紙に入りきらないときは文字を小さくします）。場面の見出し（柱）は Word と同じく罫で囲みます。A4縦 縦書きは Word と同じく場面ごとに改ページします。「PDF の書き込み欄」で、本文ページに Word と同じ書き込み用の余白と区切りの罫を付けられます（なし / 上 / 下。横書きでは上 = 左、下 = 右）。「PDF にトンボと裁ち落とし（3mm）を付ける」を入れると、角トンボ・センタートンボを付け、仕上がり（TrimBox）と裁ち落とし（BleedBox）を PDF に書き込みます。
