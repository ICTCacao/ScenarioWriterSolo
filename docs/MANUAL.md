...
- **本文**はそのまま打ちます。本文は 設定 › 書式 の「本文の文字数」（既定 32 文字、字下げのぶんは引く）で折り返します。「読む」画面やテキスト出力と同じ折り返しです。文字の大きさはスタイルのサイズと、設定 › 書式 の「基準サイズ」で決まります。
- 行の右端の「…」から、上下に行を足す・上下へ動かす・削除ができます。右クリックでも同じメニューが出ます。
...
設定 › 書式 の「編集の書き方向」か、「シナリオ › 縦書きで編集」（⌥⌘T）で切り替えます。縦書きでは行が右から左へ並び、各行の本文は上から下へ書きます。列の幅は上の選択ボックスの幅を最小にして、本文が長ければ左へ広がります。場面・登場人物・シノプシスの画面も縦書きになります。この設定は作品ファイルには入りません。
...
- **文字が小さい / 大きい**: 設定 › 書式 の「基準サイズ」で編集画面の大きさを変えられます。「読む」画面は画面のバーで。
