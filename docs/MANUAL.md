...
- **PDF**: Word と同じ用紙・書き方・表紙の情報で、Word を経由せずに直接組んだ PDF。文字はすべてアウトライン化し（フォントを埋め込まない）、色は墨 1 色なので、印刷所への版下にそのまま使えます。場面の見出し（柱）は Word と同じく罫で囲みます。A4縦 縦書きは Word と同じく場面ごとに改ページします。「PDF にトンボと裁ち落とし（3mm）を付ける」を入れると、角トンボ・センタートンボを付け、仕上がり（TrimBox）と裁ち落とし（BleedBox）を PDF に書き込みます。
