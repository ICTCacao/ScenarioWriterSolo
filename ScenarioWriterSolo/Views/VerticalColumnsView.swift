...
struct VerticalColumnsView: View {
    ...
    private func makeLayout(_ lines: [ScriptLine]) -> Layout {
        ...
        var used: CGFloat = 0
        for line in lines {
            let w = widths[line.id] ?? (Self.headerW + Self.rowPadding * 2)
            l.widths.append(w)
            used += w + Self.spacing
        }
    ...
    var body: some View {
        ...
        .onPreferenceChange(VerticalTextWidthKey.self) { measured in
            // 本文欄の実測幅 → 列幅（LineRowView.verticalBody: max(見出し幅, 本文幅) + 余白）
            var changed = false
            for (id, tw) in measured {
                let w = max(Self.headerW, tw) + Self.rowPadding * 2
    ...
    private func refreshWidths(_ lines: [ScriptLine], force: Bool) {
        ...
        for line in lines {
            ...
            newWidths[line.id] = max(Self.headerW, tw) + Self.rowPadding * 2
            newKeys[line.id] = key
            measured += 1
        }
    ...
    private func scroll(toLine id: Int64, in lines: [ScriptLine], animated: Bool) {
        ...
        if animated { withAnimation { scrollPos.scrollTo(x: x) } } else { scrollPos.scrollTo(x: x) }
    }
}
