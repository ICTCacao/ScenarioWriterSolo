...
同じ作品ファイルを読み書きする Windows 版（Tauri）が [ScenarioWriterWin/](ScenarioWriterWin/) にあります。配布物は `dist/ScenarioWriterWin-β7-windows.zip`（インストーラ・単体 exe・[マニュアル](ScenarioWriterWin/docs/MANUAL.md)）。版の呼び方は Mac 版と揃えて β7（インストーラの内部番号だけ 0.7.0）。
...
- **縦書きで編集**（設定 › 書式「編集の書き方向」、または シナリオ › 縦書きで編集 ⌥⌘T）… 行が右から左へ並び、各行の本文は上から下。Tab / ⇧Tab のほか ⌘← / ⌘→ で前後の行へ。場面・登場人物・シノプシスの画面も縦書きになる（場面説明・人物設定の既定の幅は上の入力欄と同じで、長くなると列が増える）。台本の列は見えている分だけ作るので、数百行の場面でもウインドウの大きさを変えたときに固まらない。シノプシスの縦書き欄は枠の高さで折り返す
...
- **読む** … Web 版の劇団員プレビューと同じ画面で通し読み。縦書き / 横書き、文字サイズ、場面ジャンプ。登場人物名欄と本文の幅は「設定 › 書式」の文字数に揃い、半角の英数・カナは全角で表示
- **書き出し**（⌘E）
