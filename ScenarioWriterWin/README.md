...
- **この Mac で作る**（Tauri の実験的なクロスビルド。`brew install nsis llvm`、`rustup target add x86_64-pc-windows-msvc`、`cargo install cargo-xwin` のあと）:
  ...
  # → src-tauri/target/x86_64-pc-windows-msvc/release/bundle/nsis/ScenarioWriterWin_0.5.0_x64-setup.exe（dist では ScenarioWriterWin-β5-setup.exe に改名。内部番号は数字しか使えないので 0.5.0）
