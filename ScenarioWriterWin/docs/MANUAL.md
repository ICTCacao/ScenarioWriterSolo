...
1. `ScenarioWriterWin_0.5.0_x64-setup.exe` をダブルクリックします。
...
インストールせずに使うときは `ScenarioWriterWin-0.5.0-x64-portable.exe` をそのまま起動します（Windows 10/11 の WebView2 が入っていれば動きます。この場合 `.scwd` の関連付けはされません）。
...
- **左の一覧**: 画面（シナリオ編集・場面・登場人物・シノプシス・シナリオ情報・読む・スタイル）、台本（場面ごと、行数つき）、登場人物。場面をクリックするとその場面の台本に、登場人物をクリックすると登場人物の画面に移ります。
...
- 行の右上（縦書きでは種別の隣）の「⋯」から、前後に行を足す・前後へ動かす・削除ができます。
...
| メニュー | 項目 | ショートカット |
|---|---|---|
| ファイル | 新規… / 開く… / 保存 / 別名で保存… / 作品を閉じる / 終了 | Ctrl+N / Ctrl+O / Ctrl+S / Ctrl+Shift+S / Ctrl+W |
...
## 12. Mac 版との違い（この版でまだ無いもの）

- テキスト / Word / HTML ファイルの書き出し（Mac 版で開いて書き出してください）
